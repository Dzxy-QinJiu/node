...
(function () {
    ...
    function GeminiScrollbar(config) {
        ...
        this.handleScrollBottom = function() {};
        ...
        this._scrollbarHorizontalElement = null;
        this._listenScrollBottomLock = false;
    }
    ...
    GeminiScrollbar.prototype.bindScrollBottom = function() {
        this._cache.events.handleScrollBottomHandler = this._handleScrollBottomHandler.bind(this);
        this._viewElement.addEventListener('scroll', this._cache.events.handleScrollBottomHandler);
    };

    GeminiScrollbar.prototype.unbindScrollBottom = function() {
        this._viewElement.removeEventListener('scroll', this._cache.events.handleScrollBottomHandler);
        this._cache.events.handleScrollBottomHandler = null;
    };

    GeminiScrollbar.prototype.scrollBottomUnlock = function() {
        this._listenScrollBottomLock = false;
    };

    GeminiScrollbar.prototype._handleScrollBottomHandler = function(event) {
        if(this._listenScrollBottomLock) {
            return;
        }
        var viewElement = this._viewElement;
        var viewElementHeight = $(viewElement).height();
        var viewElementScrollTop = viewElement.scrollTop;
        var viewElementScrollHeight = viewElement.scrollHeight;
        if((viewElementHeight + viewElementScrollTop) >= (viewElementScrollHeight)) {
            this._listenScrollBottomLock = true;
            this.handleScrollBottom();
        }
    };

    //检测是否要下拉加载下一页，用于高清屏分辨率
    GeminiScrollbar.prototype._detectLoadNextPage = function() {
        if(this._listenScrollBottomLock) {
            return;
        }
        var $viewEle = $(this._viewElement);
        //是否有selector，用来判断内容为空，但是自动检测的问题
        var itemCssSelector = this.itemCssSelector;
        if(itemCssSelector) {
            //找到满足selector的元素
            var $items = $viewEle.find(itemCssSelector);
            //如果没有这样的元素，则不进行检查
            if(!$items.length) {
                return;
            }
        ...
        if(
            viewEleOffset.top <= (loadingOffset.top + $loading.height()) &&
            loadingOffset.top <= (viewEleOffset.top + $viewEle.height())
        ) {
            this._listenScrollBottomLock = true;
            setTimeout(() => {
                this.handleScrollBottom();
