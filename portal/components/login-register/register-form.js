...
class RegisterForm extends React.Component {
    ...
    openUserAgreement = (e) => {
        ...
        pcAndWechatMiniPrivicyAndAgreement('/user/agreement');
    }
    openPrivacyPolicy = (e) => {
        Trace.traceEvent(e, '点击隐私策略');
        pcAndWechatMiniPrivicyAndAgreement('/privacy/policy');
    }
    toLogin = (eventTraceDescr, e) => {
        Trace.traceEvent(e, eventTraceDescr);
        let search = window.location.search || '';
        var isRegistry = false;
        if(search.indexOf('isFromHome') !== -1){//从小程序的home页面跳转到注册页才加上这个字段
            isRegistry = true;
        }
