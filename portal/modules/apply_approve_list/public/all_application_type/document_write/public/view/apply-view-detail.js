...
import {transferBtnContent,renderStepContent,getApplyListDivHeight} from 'MOD_DIR/apply_approve_list/public/utils/apply_approve_utils';
class ApplyViewDetail extends React.Component {
    ...
    getBusinessApplyDetailData(detailItem, applyData) {
        setTimeout(() => {
            ...
            if (_.includes(APPLY_FINISH_STATUS, detailItem.status)) {
                ...
                DocumentWriteApplyDetailAction.getApplyCommentList({id: detailItem.id});
                //根据申请的id获取申请的节点位置
                DocumentWriteApplyDetailAction.getApplyTaskNode({id: detailItem.id});
    ...
    addReply = (e,callback) => {
        ...
        //提交数据
        DocumentWriteApplyDetailAction.addApplyComments(submitData,callback);
    };
    //备注 输入框改变时候触发
    ...
    renderDetailBottom() {
        ...
        if (detailInfoObj.status === 'ongoing' && showApproveBtn){
            ...
            //或者 clickConfirmBtn 是true 是表示已经确认过 待确认申请
    ...
    renderApplyApproveSteps =() => {
        ...
        if (_.get(candidate,'[0]')) {
            ...
            if (hasApprovedReportAndDocumentApply(this)){
    ...
    render() {
        ...
        return (
            <div className={detailWrapCls} style={{'width': this.props.width, 'height': this.props.height}} data-tracename="文件撰写审批详情界面">
                <ApplyDetailStatus
