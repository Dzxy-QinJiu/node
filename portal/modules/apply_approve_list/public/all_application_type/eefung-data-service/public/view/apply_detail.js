 ...
 */
var ApplyDetailStore = require('../store/apply_detail_store');
        ...
        var showApplyInfo = [
            {
                ...
                renderText: () => {
                    var fileItem = _.get(detailInfo, 'detail.apply_request_files[0]');
                    var fileName = _.get(fileItem,'file_name');
                    var fileId = _.get(fileItem,'file_id');
                    const reqData = {
                        file_dir_id: _.get(fileItem,'file_dir_id'),
                        file_id: fileId,
                        file_name: fileName,
                    };
                    return (
                        <span className="file-name">{fileId ? <a href={'/rest/dataservice/download/' + JSON.stringify(reqData)}>{fileName}</a> : fileName}
                        </span>
                    );
                }
            }];
