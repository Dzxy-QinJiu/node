 ...
 */
var ApplyDetailAction = require('../action/apply_detail_action');
...
function ApplyDetailStore() {
    //初始化state数据
    this.setInitState();
    this.bindActions(ApplyDetailAction);
}
ApplyDetailStore.prototype.setInitState = function() {
    //选中的审批条目
...
ApplyDetailStore.prototype.setDetailInfoObjAfterAdd = function(detailObj) {
...
//设置某条申请的回复列表
ApplyDetailStore.prototype.setApplyComment = function(list) {
    this.replyListInfo = {
        result: '',
...
ApplyDetailStore.prototype.setInitialData = function(obj) {
    //重置数据
    this.setInitState();
    //指定详情条目
...
//获取审批详情
ApplyDetailStore.prototype.getLeaveApplyDetailById = function(obj) {
    if (obj.error) {
...
ApplyDetailStore.prototype.getLeaveApplyCommentList = function(resultObj) {
    //回复列表
...
ApplyDetailStore.prototype.setApplyFormDataComment = function(comment) {
    this.replyFormInfo.comment = comment;
};
ApplyDetailStore.prototype.cancelApplyApprove = function(resultObj) {
    if (resultObj.loading){
...
ApplyDetailStore.prototype.hideCancelBtns = function() {
    this.selectedDetailItem.showCancelBtn = false;
    this.detailInfoObj.info.showCancelBtn = false;
};
ApplyDetailStore.prototype.showOrHideApprovalBtns = function(flag){
    this.selectedDetailItem.showApproveBtn = flag;
    this.detailInfoObj.info.showApproveBtn = flag;
};
ApplyDetailStore.prototype.setNextCandidate = function(candidateArr){
    this.candidateList = candidateArr;
    checkIfLeader(candidateArr,(isLeader) => {
        this.isLeader = isLeader;
    });
};
ApplyDetailStore.prototype.setNextCandidateName = function(candidateName){
    this.detailInfoObj.info.nextCandidateName = candidateName;
};

ApplyDetailStore.prototype.hideReplyCommentEmptyError = function() {
    this.replyFormInfo.result = '';
    this.replyFormInfo.errorMsg = '';
};
//显示回复输入框为空的错误
ApplyDetailStore.prototype.showReplyCommentEmptyError = function() {
...
ApplyDetailStore.prototype.addLeaveApplyComments = function(resultObj) {
    //回复表单
...
//提交审批
ApplyDetailStore.prototype.approveLeaveApplyPassOrReject = function(obj) {
    if (obj.loading) {
...
ApplyDetailStore.prototype.cancelSendApproval = function() {
    this.applyResult.submitResult = '';
    this.applyResult.errorMsg = '';
    this.backApplyResult.submitResult = '';
    this.backApplyResult.errorMsg = '';
};
ApplyDetailStore.prototype.hideApprovalBtns = function() {
    this.selectedDetailItem.showApproveBtn = false;
    this.selectedDetailItem.showCancelBtn = false;
};
ApplyDetailStore.prototype.getNextCandidate = function(result) {
    if (result.error){
        this.candidateList = [];
    }else{
...
ApplyDetailStore.prototype.setNextCandidateIds = function(candidateId) {
    this.detailInfoObj.info.nextCandidateId = candidateId;
};
ApplyDetailStore.prototype.transferNextCandidate = function(result) {
    if (result.loading) {
...
ApplyDetailStore.prototype.getApplyTaskNode = function(result){
    if (result.error) {
        this.applyNode = [];
    } else {
...
module.exports = alt.createStore(ApplyDetailStore, 'ApplyDetailStore');
