...
var CallRecordAnalyis = React.createClass({
    ...
    getInitialState: function () {
        ...
        return {
            ...
            selectRadioValue: 'count', // 通话时长和数量切换的Radio
    ...
    getTeamMemberParam() {
        ...
        if (this.state.firstSelectValue == LITERAL_CONSTANT.TEAM && this.state.teamList.list.length > 1) { // 团队时
           if(this.state.secondSelectValue !==  LITERAL_CONSTANT.ALL) { // 具体团队时
               params.sales_team_id = (this.state.secondSelectValue).join(',');
           }
        } else { // 成员时
            if (this.state.secondSelectValue ==  LITERAL_CONSTANT.ALL ) { // 全部时
                ...
                params.user_id =  (this.state.secondSelectValue).join(','); // 成员
            }
        }
        return params;
    },
    ...
    refreshCallAnalysisData(params) {
        ...
        let data = _.clone(reqBody);
        data = _.extend(data, {
            filter_invalid_phone: "false"
        });
        this.getCallRate(data); //客服电话统计
    },
    ...
    handleSelectRadio(e) {
        if (event.target.value == 'count') {
            Trace.traceEvent(e, '点击通话数量按钮');
            this.setState({
                selectRadioValue: 'count'
            });
        } else {
            Trace.traceEvent(e, '点击通话时长按钮');
            this.setState({
                selectRadioValue: 'duration'
            });
        }
    },
            ...
            <div>
                <div className="duration-count-radio clearfix">
                    <RadioGroup onChange={this.handleSelectRadio} value={this.state.selectRadioValue}>
                        <Radio value="count"  defaultChecked="true">{Intl.get('sales.home.call.cout', '通话数量')}</Radio>
                        <Radio value="duration">{Intl.get('call.record.call.duration', '通话时长')}</Radio>
                    </RadioGroup>
                </div>

                {
                    this.state.selectRadioValue == 'count' ?
                        // 通话数量
    ...
    renderCallInfo() {
        ...
        return (
            <Table dataSource={this.state.salesPhoneList}
    ...
    renderCallAnalysisView: function () {
        ...
        return (<div className="call-table-container" ref="phoneList">
            ...
            <div style={{height:tableHeight}}>
                <GeminiScrollBar>
                    <div className="call-info col-xs-12">{this.renderCallInfo()}</div>  {/**通话率*/}
