...
import {Alert,Table,Input,Button,Icon,Checkbox, Select, message, Popconfirm} from 'antd';
const Option = Select.Option;
import GeminiScrollBar from 'CMP_DIR/react-gemini-scrollbar';
import TableUtil from 'CMP_DIR/antd-table-pagination';
...
const CallRecord = React.createClass({
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.state.callRecord.total && this.state.callRecord.data_list.length === this.state.callRecord.total && !this.state.isNoMoreTipShow) {
            //滚动条区域容器
            const scrollWrap = $(".call_record_wrap .gm-scroll-view");
            //滚动条区域内容
    ...
    onSelectFilterObj(filterKey, value){
        ...
        if (value == CALL_TYPE_OPTION.PHONE){
            this.state.callType = <i className="iconfont icon-call-back"></i>;
        } else if (value == CALL_TYPE_OPTION.APP) {
            this.state.callType = <i className="iconfont icon-ketao-app"></i>;
        } else if (value == CALL_TYPE_OPTION.ALL) {
            this.state.callType = <i className="iconfont  icon-all"></i>;
        }
   ...
   filterTypeStatusKeySelect(filterKey, columnLabel){
    ...
    handleAudioPlay: function (item) {
        ...
        //录音的地址
        var playItemAddr = "/record/" + item.recording + ".mp3";
        this.setState({
        ...
        return [
            {
                ...
                className: this.state.isFilter ? 'call-user' : 'has-filter call-user'
            }, {
                ...
                render: (callState, column) => {
                    ...
                    return <div>
                        {CALL_STATUS_MAP[callState]}
                        {
                            ...
                            column.recording ? <i className={cls} onClick={this.handleAudioPlay.bind(this, column)}
                ...
                key: 'holding_time',
                sorter: true,
                className: 'has-filter data-float-right',
                ...
                sorter: true,
                width: "140px",
                className: 'has-filter call-time',
                render: (time) => {
                        ...
                        <div>
                            {column.customer_name ? (
                                <div title={Intl.get("call.record.customer.title", "点击可查看客户详情")}>
                                    {column.customer_name}
                                    <input type="hidden" value={column.dst} className="phone_hidden"/>
                                    <input type="hidden" value={column.customer_name} className="customer_name_hidden"/>
                                </div>
                            ) : (
                                <div title="点击添加客户和联系人">
                                    <Icon type="plus" onClick={this.showAddCustomerForm.bind(this, column.dst)}/>
                                </div>
                            )}
                        </div>
                    )
                    ...
                    return (
                        <div className="add-content">
                            <Popconfirm title={Intl.get("call.record.is.save.content.title", "是否保存跟进内容？")}
                                        visible={column.confirmVisible}
                                        onConfirm={this.handleContentSubmit.bind(this,column.id)}
                                        onCancel={this.cancelConfirm.bind(this, column.id, column.remark)}
                                        okText={Intl.get("user.yes", "是")}
                                        cancelText={Intl.get("user.no", "否")}
                            >
                                <input
                                    defaultValue={column.remark}
                                    onBlur={this.toggleConfirm.bind(this, column.id, column.remark)}
                                    type="text"
                                    id={"content" + column.id}
                                    onKeyUp={this.checkEnter.bind(this, column.id)}
                                />
                            </Popconfirm>
                        </div>
                    )
    ...
    checkEnter(id) {
        if (event.keyCode == 13) {
            $(".custom-tbody #content" + id).blur();
        }
    },

    // 失去焦点后，触发确认对话框
    toggleConfirm(id, sourceValue) {
        let value = $(".custom-tbody #content" + id).val();
        if (sourceValue) { // 有内容时，对应的是修改
            if (value == sourceValue) { // 没做修改，直接返回，不出现确认框
                return;
            } else { // 修改内容时，出现确认框
                CallRecordActions.toggleConfirm({id: id, flag: true});
            }
        } else { // 添加跟进内容时
            if ( value && value.trim()) {
                CallRecordActions.toggleConfirm({id: id, flag: true});
    ...
    // 确认框点击不保存时
    cancelConfirm(id, sourceValue){
        Trace.traceEvent(this.getDOMNode(), '是否保存编辑的跟进内容，点击否');
        let value = $(".custom-tbody #content" + id).val();
        if (sourceValue) { // sourceValue是跟进内容原有的值，当有内容时
            $(".custom-tbody #content" + id).val(sourceValue);
        } else {
            $(".custom-tbody #content" + id).focus();
        }
        CallRecordActions.toggleConfirm({id: id, flag: false}); // 确认框关闭
    },

    // 编辑跟进内容的提交
    handleContentSubmit(id) {
        Trace.traceEvent(this.getDOMNode(), '是否保存编辑的跟进内容，点击是');
        CallRecordActions.toggleConfirm({id: id, flag: false});
        let value = $(".custom-tbody #content" + id).val();
        var queryObj = {
            remark: value,
            id: id
    ...
    selectFilterPhone(e) {
        ...
        if(status) {
            Trace.traceEvent(e, '过滤小于7位的号码');
        } else {
            Trace.traceEvent(e, '取消过滤小于7位的号码');
        }

        CallRecordActions.filterPhone(status);
        setTimeout(() => {
            this.getCallRecommendList();
            this.getCallListByAjax();
        });
                ...
                <div className="call_record_content">
                    <TopNav>
                        ...
                        <div className="filter-phone-button">
                            <Checkbox
                                title={Intl.get("call.record.filter.tip", "过滤小于7位的号码，如114、12580...")}
                                checked={this.state.filter_phone}
                            ...
                            <span className="refresh-btn">
                                <i className="iconfont  icon-refresh refresh"  title={Intl.get(" common.refresh", "刷新")}
                                   onClick={this.handleRefresh} data-tracename="点击刷新按钮"></i>
                            </span>
                    ...
                    <div className="call_record_wrap splice-table">
                        <div className="table-thead custom-thead" ref="thead">
                            <Table
    ...
    getCallListByAjax(queryParam) {
        var queryObj = {
            ...
            filter_phone: this.getReqParam(queryParam, 'filter_phone')
        };
    ...
    renderCallRecordContent() {
        ...
        return <div className="table-tbody custom-tbody">
            <Table
