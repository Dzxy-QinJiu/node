...
const OnlineUserList = React.createClass({
    ...
    alignTheadTbody: function () {
        TableUtil.alignTheadTbody(this.refs.table);
    },
    componentDidMount: function () {
        $('body').css("overflow","hidden");
        this.alignTheadTbody();
        $(window).on("resize", this.alignTheadTbody);
    ...
    componentWillUnmount: function () {
        ...
        $(window).off("resize", this.alignTheadTbody);
    ...
    render: function () {
        ...
        const columns = [
            {title: Intl.get("common.username", "用户名"), dataIndex: "user_name", className: 'show-user-detail', key: "user_name", width: "10%",render : function($1,row) {
                return <div>{row.user_name}<input type="hidden" value={row.user_id} className="user_id_hidden"/></div>
            }},
            {title: Intl.get("common.nickname", "昵称"), dataIndex: "nick_name", className: 'show-user-detail', key: "nick_name", width: "10%"},
            {title: "IP", dataIndex: "ip", className: 'show-user-detail', key: "ip", width: "10%"},
            {title: Intl.get("common.ip.location", "IP归属地"), dataIndex: "ip_address", className: 'show-user-detail', key: "ip_address", width: "10%"},
            {title: Intl.get("user.user.type", "用户类型"), dataIndex: "tags", className: 'show-user-detail', key: "tags", width: "10%",render : function($1,row) {
            ...
            {title: Intl.get("user.overdue.whether", "是否过期"), dataIndex: "is_expired", className: 'show-user-detail',key: "is_expired", render: this.renderStatusColumn, width: "80px"},
            {title: Intl.get("user.online.login.time", "登录时间"), dataIndex: "login_at",className: 'show-user-detail', key: "login_at", render: this.renderLoginTimeColumn, width: "140px"},
            {title: Intl.get("common.client", "客户端"), dataIndex: "user_agent",className: 'show-user-detail', key: "user_agent"},
            {title: Intl.get("common.operate", "操作"), dataIndex: 'operate', key: 'operate',width: '5%',  render: function (text, record, index) {
                return (
        ...
        return (
            <div className="list-content" ref="table" data-tracename="在线用户列表">
                {
