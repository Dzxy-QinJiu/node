...
var AntTableCommon = require("../../../../components/antd-table-pagination");
...
var UserTabContent = React.createClass({
    ...
    removeCurrentRowClass : function() {
        $(this.refs.tableWrap).find("tr.current_row").removeClass("current_row");
    },
    //更新用户基本信息
    ...
    bindEventEmitter : function(){
        ...
        //清除当前选中行样式
        AppUserUtil.emitter.on(AppUserUtil.EMITTER_CONSTANTS.REMOVE_CURRENT_ROW_CLASS , this.removeCurrentRowClass);
    ...
    unbindEventEmitter : function() {
        ...
        //清除当前选中行样式
        AppUserUtil.emitter.removeListener(AppUserUtil.EMITTER_CONSTANTS.REMOVE_CURRENT_ROW_CLASS , this.removeCurrentRowClass);
        //更新一个用户的一个应用成功后，同步列表中的数据
    ...
    componentDidMount : function() {
        ...
        AntTableCommon.zoomInSortArea(this.refs.userListTable);
    ...
    getAccountTypeList : function(apps,rowData) {
        var cls = classNames({
            app_ellipsis : apps.length > 1
        });
        var appList = apps.map(function(app, i) {
            var text = AppUserUtil.getUserTypeText(app.user_type);
            return (
    ...
    getTimeList : function(field,rowData) {
        var apps = rowData.apps || [];
        var cls = classNames({
            app_ellipsis : apps.length > 1
        });
        var appList = apps.map(function(app, i) {
            var time = moment(new Date(+app[field])).format(oplateConsts.DATE_FORMAT);
    ...
    getAppStatusList : function(apps,rowData) {
        var cls = classNames({
            app_ellipsis : apps.length > 1
        });
        var appList = apps.map(function(app, i) {
        ...
        var columns = [
            {
                ...
                render : function($1,rowData,idx) {
                    ...
                    return (
                        <div title={user_name}>
                            {isShown ? <i className="iconfont icon-warn-icon unnormal-login" title="异地登录告警"></i> : null}
                            {user_name}
            ...
            { 
                ...
                key : 'status',
                render : function(apps,rowData,idx) {
                    return _this.getAppStatusList(apps,rowData);
                }
            },
            {
                ...
                key : 'accountType',
                render : function(apps,rowData,idx) {
                    return _this.getAccountTypeList(apps,rowData);
                }
            },
            {
                ...
                sorter: sortable,
                render : function($1,rowData,idx) {
                    return _this.getTimeList('create_time',rowData);
                }
            },
            {
                ...
                sorter: sortable,
                render : function($1,rowData,idx) {
                    return _this.getTimeList('end_time',rowData);
                }
            },
            {
    ...
    renderFilterFields : function() {
        ...
        return <div style={{display : this.state.filterRoles.selectedRole ? 'none' : 'block'}}>
            ...
            {Oplate.hideUserManageItem ?  null : this.renderFilterTeamName()}
    ...
    renderTableBlock : function() {
        ...
        //管理员可以批量操作
        //销售可以批量操作
        var rowSelection = hasPrivilege(AppUserUtil.BATCH_PRIVILEGE.ADMIN) ||
                           hasPrivilege(AppUserUtil.BATCH_PRIVILEGE.SALES)
                            ? this.getRowSelection() : null;
        ...
        return (
            <div className="user-list-table-wrap splice-table" style={{display : doNotShow ? 'none' : 'block'}}>
                ...
                <div className="user-list-tbody custom-tbody" style={{height:divHeight}} ref="tableWrap">
                    <GeminiScrollBar
                        ...
                        itemCssSelector=".ant-table-tbody>tr"
                    >
                        <Table
                            dataSource={this.state.appUserList}
                            rowSelection={rowSelection}
