...
class ApplyViewDetail extends React.Component {
    ...
    componentDidMount() {
        ...
        if (_.get(this.props,'detailItem.afterAddReplySuccess')){
            setTimeout(() => {
                ReportSendApplyDetailAction.setDetailInfoObj(this.props.detailItem);
            });
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (_.get(nextProps,'detailItem.afterAddReplySuccess')){
            setTimeout(() => {
                ReportSendApplyDetailAction.setDetailInfoObj(nextProps.detailItem);
            });
    ...
    getBusinessApplyDetailData(detailItem) {
        setTimeout(() => {
            ...
            if (detailItem.status === 'pass' || detailItem.status === 'reject') {
                ReportSendApplyDetailAction.getLeaveApplyCommentList({id: detailItem.id});
                ReportSendApplyDetailAction.getLeaveApplyDetailById({id: detailItem.id}, detailItem.status);
            } else if (detailItem.id) {
                ReportSendApplyDetailAction.getLeaveApplyDetailById({id: detailItem.id});
                ReportSendApplyDetailAction.getLeaveApplyCommentList({id: detailItem.id});
                //根据申请的id获取申请的状态
    ...
    addReply = (e) => {
        ...
        //提交数据
        ReportSendApplyDetailAction.addLeaveApplyComments(submitData);
    };
    //备注 输入框改变时候触发
    ...
    passOrRejectApplyApprove = (confirmType) => {
        var detailInfoObj = this.state.detailInfoObj.info;
        ReportSendApplyDetailAction.approveLeaveApplyPassOrReject({id: detailInfoObj.id, agree: confirmType});
    };
