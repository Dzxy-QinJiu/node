...
function ReportSendApplyActions() {
    ...
    this.getAllApplyList = function(queryObj) {
        ...
        DocumentWriteAjax.getWorklistApplyList({type: APPLY_APPROVE_TYPES.OPINIONREPORT}).then((workList) => {
            ...
            DocumentWriteAjax.getAllApplyList(queryObj).then((data) => {
                ...
                this.dispatch({
                    error: true,
                    loading: false,
                    errMsg: errorMsg || Intl.get('apply.failed.get.type.application', '获取全部{type}申请失败', {type: Intl.get('apply.approve.document.writing', '文件撰写')})
                });
            });
            ...
            this.dispatch({
                error: true,
                loading: false,
                worklistErrMsg: errorMsg || Intl.get('apply.failed.get.my.worklist.application', '获取由我审批的{type}申请失败', {type: Intl.get('apply.approve.document.writing', '文件撰写')})
            });
        });
    };
}
