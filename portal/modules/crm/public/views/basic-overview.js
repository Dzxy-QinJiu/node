...
import {isClueTag, isTurnOutTag, isHasRevisitTag} from "../utils/crm-util";
...
var BasicOverview = React.createClass({
    ...
    getRecommendTags: function () {
        batchAjax.getRecommendTags().then(data => {
            if (_.isArray(data.result) && data.result.length) {
                ...
                let recommendTags = _.filter(data.result, tag => !isClueTag(tag) && !isTurnOutTag(tag) && !isHasRevisitTag(tag));
    ...
    saveEditTags: function (tags, successFunc, errorFunc) {
        // 保存前先过滤掉线索、转出、已回访标签
        tags = _.filter(tags, tag => !isClueTag(tag) && !isTurnOutTag(tag) && !isHasRevisitTag(tag));
        let submitData = {
