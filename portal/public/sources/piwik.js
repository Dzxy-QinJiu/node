    ...
    window.Piwik = (function () {
        ...
        function Tracker(trackerUrl, siteId) {
            ...
            var
                ...
                //window opened
                tabOpened = true,
                //use window  status ,when set to true and tabOpened is true,request will always sended. others will depend on focus.
                useTabStatus = false,
                // app info
                appData = {},
            ...
            /*
             * Modify beat type，always beat or depend on focus
             *
             */
            function updateBeatType(){
                //如果使用tab存在状态，决定发送定时数据，则取消focus和blur监听
            ...
            this.setTabOpened = function (opened) {
                tabOpened = opened;
                updateBeatType();
            };
            this.enableUseTabStatus = function (enable) {
                useTabStatus = enable;
                updateBeatType();
            };
            //set appData
            this.setAppData = function (key_or_obj, opt_value) {
                if (isObject(key_or_obj)) {
                    appData = key_or_obj;
                } else {
                    if (!appData) {
